      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 relative z-10">
        {features.map((feature, index) => (
          <div key={index} className="group relative">
            ...
            {/* Card */}
            <div className="h-full bg-slate-950/80 border border-primary-500/20 p-6 rounded-2xl backdrop-blur-md transition-all duration-500 hover:scale-105 hover:border-primary-500/50 hover:shadow-[0_10px_40px_-10px_rgba(6,182,212,0.2)] flex flex-col items-center text-center group-hover:-translate-y-2 relative overflow-hidden">

              {/* --- LASER SCAN EFFECT --- */}
              <div className="absolute top-0 left-0 w-full h-[5px] bg-gradient-to-r from-transparent via-primary-400 to-transparent opacity-0 group-hover:opacity-100 group-hover:animate-scan z-20"></div>
              ...
              {/* Icon Container */}
              <div className="w-14 h-14 rounded-full border border-primary-500/40 bg-slate-900/50 flex items-center justify-center mb-6 shadow-[0_0_20px_rgba(6,182,212,0.15)] group-hover:shadow-[0_0_30px_rgba(6,182,212,0.3)] group-hover:border-primary-400 transition-all relative shrink-0">
                <div className="text-primary-400 group-hover:text-white transition-colors">
                  {feature.icon}
                </div>
              </div>

              <h3 className="text-lg font-bold text-white mb-3 group-hover:text-primary-300 transition-colors leading-tight relative z-10">
                {feature.title}
              </h3>
              <p className="text-sm text-slate-400 leading-relaxed group-hover:text-slate-300 relative z-10">
                {feature.desc}
              </p>
            </div>
          </div>
